...
- Dual-architecture Area-31 run `36139333328` remains valid historical external evidence, but it is not the current promotion result. Full runs `36216084164` and `36218288072` encountered stale project-owned DNIVHT/DNIVTK command procedures on discovered QCOCAL. The proof now performs bounded cleanup of only those ephemeral names, re-lists after each DAP erase, and still requires normal post-test deletion; current Area-31 promotion status therefore remains pending until an exact-head rerun is green.
- Cross-runner VDE2 and current Area-31 requalification are the remaining Phase 8 exit dependencies. The five `VDE_SSH_*` rendezvous secrets are present. Run `36151198844` on `5d1f1a6e40961d86b540eabde9bcaf6559e863bd` isolates the current failure to the server runner before the reverse tunnel is published: the ephemeral inner sshd rejects its authenticated loopback selfcheck. The harness now enables PAM for that private sshd so GitHub's locked `runner` password entry does not pre-empt public-key authentication; password authentication remains disabled and the forced-command key restriction remains in force.
