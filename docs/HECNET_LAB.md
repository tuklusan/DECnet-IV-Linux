...
- Cross-runner VDE2 is the sole remaining Phase 8 exit dependency. The five `VDE_SSH_*` rendezvous secrets are present. Run `36147917346` on `90679867da8fd3426b812d90db845eeda013a19f` now fails bounded with `Connection timed out during banner exchange`: secret/key preflight passes and the server reverse session remains alive, but the client does not receive the ephemeral inner sshd banner through the reverse endpoint. The next gate verifies the inner sshd locally first and separately requires the raw SSH banner through the bastion before inner authentication, keeping probe processes kill-bounded without shortening the long-lived VDE bridge.
