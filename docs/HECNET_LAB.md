...
- Cross-runner VDE2 is the sole remaining Phase 8 exit dependency. The five `VDE_SSH_*` rendezvous secrets are now present: run `36142699326` passed preflight on both runners and reached the real SSH proof. Its server kept the reverse SSH process alive but timed out waiting for the client completion marker; the client stayed inside its SSH join step until the 25-minute job bound. The proof now bounds outer and inner SSH connection attempts and preserves the final readiness error so the next exact run can expose the concrete rendezvous failure instead of hanging.
