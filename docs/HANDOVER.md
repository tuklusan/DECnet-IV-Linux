...
Resume point:
Candidate `8b08c2873e0b0914e5a1e33005d5ab229125e133` was rejected by all four VM gates in workflow run `34963593635`; every non-VM gate was green. AArch64 evidence showed the FAT `cidata` device present after boot but Tiny Cloud never executed, while the x86 TCG case exceeded its 600-second completion window. The repaired candidate forces Tiny Cloud enablement through QEMU SMBIOS product serial `ds=nocloud`, keeps the labeled seed as the metadata source, gives x86 a configurable 1200-second default completion window, and retains compact x86 logs/pcap alongside the resumable session. The implementation still stops at Routing Layer EtherType plumbing: no adjacency, routing, NSP, Session Control, NICE/NML application behavior, or DDCMP is claimed yet. The retained recovery source remains workflow run `34948868114`, session `gha-34948868114`.
