...
5. Perform substantive work directly on `main`. Every substantive commit must refresh both `docs/PROJECT_STATE.md` and `scratch/RESUME.md` in the same commit. Acceptance applies only to the exact unchanged `main` commit that passes the required mechanical and protocol gates.
...
7. There is no manual delivery-pass counter or multi-pass review prerequisite. Candidate promotion is determined by the documented exact-SHA acceptance gates.
