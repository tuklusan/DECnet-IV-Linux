...
Status: active at external requalification. Local VDE2 and MULTINET remain green. Older dual-architecture Area-31 evidence at `36139333328` is retained as historical proof, but exact-head full runs `36216084164` and `36218288072` exposed stale project-owned QCOCAL command procedures, so Area-31 must pass again after bounded DNIVHT/DNIVTK lifecycle cleanup. Cross-runner VDE2 run `36218217564` proved the initial marked frame, Route20/PyDECnet adjacency, hard transport cut, marked-frame loss and adjacency expiry; recovery still needs to prove the rebuilt Ethernet path, second adjacency, client-switch restart and third frame. Credentials/endpoints remain GitHub Actions secrets. Phase 9 must not start until Area-31 and the complete two-runner VDE2 proof are green on one exact unchanged candidate.
