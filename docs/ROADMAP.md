...
Status: active at the final cross-runner dependency. Local VDE2, MULTINET and exact-head dual-architecture Area-31 are green; Area-31 run `36219353713` on `bc95d47b44a8ce9ba43411a4e674add2ea0b5e31` confirms the bounded QCOCAL DNIVHT/DNIVTK cleanup. Cross-runner VDE2 run `36218217564` already proved the initial marked frame, Route20/PyDECnet adjacency, hard transport cut, marked-frame loss and adjacency expiry. Run `36219283281` then failed earlier during direct persistent bastion-forward bootstrap. Only direct outer-connect/outer-banner timeouts may select the automatic alternate-runner path; all inner transport, frame, adjacency and recovery checks remain strict. Credentials/endpoints remain GitHub Actions secrets. Phase 9 must not start until the complete two-runner VDE2 proof is green on one exact unchanged candidate.
