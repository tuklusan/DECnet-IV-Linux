...
`tools/project_state_gate.py` validates staged continuity records from the staged index and committed continuity records from the requested commit object. Working-tree contents cannot substitute for either source. `tools/workflow_budget_gate.py` applies the same source discipline: pre-commit checks read the staged index and acceptance checks read the requested commit object. It enforces the 75-minute ceiling, bounded retention, `queue: max`, immutable GitHub action pins, parent expected-SHA plumbing, complete paginated VM-checkpoint pruning, compact VM exclusions and maximum-two-scenario interoperability rows. `tests/policy/test_image_builder_gate.py` follows the same discipline: the pre-commit hook reads the staged image builder and workflow SoP reads the requested commit object, preventing a good mutable working copy from masking a bad staged/committed direct-boot builder. The image-builder gate also requires installed smoke-byte equality, systemd unit validation, arm64 direct-boot decompression/header checks, uncompressed acceptance QCOW2 construction and a post-conversion critical-file checksum round trip.
...
The candidate-image builder verifies the archived smoke script/unit against their installed guest copies, validates the unit with `systemd-analyze`, and after QCOW2 conversion round-trips the image to raw and rechecks both critical-file hashes. Acceptance base images are intentionally left uncompressed until correctness is established. For arm64 direct boot, a gzip-packaged Ubuntu `vmlinuz` is decompressed before QEMU use and the mandatory AArch64 Image magic is checked; amd64 retains its packaged direct-boot image.
...
Three consecutive complete semantic/manual SoP passes then completed cleanly on candidate `a57f44a2400bbf1fce7cc411489d51182c9ccd95`, and acceptance parent run `35181684059` dispatched exact-SHA-bound child gates. Native x86_64/aarch64 builds and project-state validation were green, and the Route20 reference baseline was green. VM child run `35181712924` exposed an image-construction defect before protocol boot on arm64: the kernel package installed, but `--no-install-recommends` left no initramfs generator, so no `/boot/initrd.img-*` existed. The correction explicitly installed `initramfs-tools`, asserted the exact generated initrd before package cleanup, and added `tests/policy/test_image_builder_gate.py` to local/workflow gates. The first review of that correction then found the local hook called the new gate without staged-source selection; the gate gained explicit `--staged` and `--tree` modes, and both call sites use them.

The first full semantic/manual pass on the exact-source correction candidate completed cleanly. The adversarial second pass then found a workflow-storage defect: successful VM checkpoint pruning listed only the first 100 repository artifacts, so old same-architecture checkpoints beyond the first API page could survive indefinitely despite the documented rolling-newest policy. The VM workflow now paginates the artifact listing before filtering/deleting stale checkpoints, and `tools/workflow_budget_gate.py` requires both the artifact-list endpoint and pagination safeguard.

Candidate `a122e8e82541a9499b9b514568b70e77de97ca4b` then completed three consecutive clean full semantic/manual SoP passes. Exact-head acceptance parent `35184898090` dispatched children bound to that SHA. Repository/continuity, native x86_64 and arm64 builds, and pinned reference baselines were green. E1 child run `35184925391` failed on both architectures before any DECnet protocol assertion. On amd64 both VMs reached Ubuntu userspace, but systemd read the candidate smoke unit as a long sequence of assignments outside any section and refused the service for lack of `ExecStart`; a rerun reproduced the same gate failure. The tracked source unit itself remained the expected 1023-byte unit, so acceptance exposed a base-image integrity gap rather than a protocol result. On arm64 both serial logs remained zero bytes for the full 300-second gate; the packaged Ubuntu arm64 `vmlinuz` is gzip-compressed while AArch64 direct boot requires the decompressed Image. No result from that acceptance run is eligible for Phase 3 promotion.

The image correction now makes both failures fail-fast and testable: critical smoke bytes are compared before conversion and again after a QCOW2-to-raw verification round trip; `systemd-analyze verify` must accept the installed unit; `qemu-img check` must accept the candidate image; acceptance images are converted without qcow2 compression; and arm64 gzip kernels are decompressed and checked for the mandatory AArch64 Image header before QEMU launch. The policy regression requires each safeguard. This change resets semantic/manual SoP to zero.
