...
Phase 3 remains active. The current candidate contains the direct-boot initramfs correction, exact staged/committed image-builder regression source selection, and paginated VM-checkpoint pruning with workflow-policy enforcement. All prior SoP counts and acceptance evidence are invalid after the latest workflow correction. No current-candidate semantic/manual pass is yet counted.
