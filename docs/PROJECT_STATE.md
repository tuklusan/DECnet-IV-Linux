...
Acceptance candidate `0ea6d28f4f77ca156a2dbc04bffec83034fd2723` proved the intended infrastructure boundary in execution: repository policy and both native builds were green; both architecture foundations were built/saved and subsequently restored by interoperability jobs; exact-candidate injection completed without guest package installation; amd64 guests booted and reached live DECnet adjacency. The remaining failures exposed bounded harness issues rather than a return to image provisioning: aligned `dnctl stats` output was parsed too strictly, QEMU vvfat appeared as partition `/dev/vdb1` rather than the whole disk, and hosted ARM64 direct boot needed a larger/explicit `virt` configuration. These corrections do not alter the persistent foundation fingerprint.
...
Phase 3 is the active workstream. Infrastructure provisioning is no longer the project focus: `outer-v2` persistence, disposable exact-candidate layers and short-path QMP execution have been demonstrated. The immediate acceptance rerun is only to validate the three bounded harness/direct-boot corrections revealed by the first `outer-v2` execution; any subsequent DECnet protocol failure is a Phase 3 implementation issue and should be debugged as such.

## Next action

Run one exact-head acceptance after the stats parser, vvfat partition selection and ARM64 `virt` boot corrections. Reuse the existing `outer-v2` foundations. If the gates reach a substantive DECnet failure, continue Phase 3 from that failure according to `docs/ROADMAP.md`; do not return to infrastructure redesign unless a gate provides concrete evidence that the persistent foundation mechanism itself is defective.
