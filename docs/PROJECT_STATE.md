...
- The policy workflow has an owner-only demand-driven control path: an issue opened by the repository owner with the exact title `DNIV acceptance gates` may dispatch the manual build, continuity, reference and E1 workflows only after repository policy passes and only if `main` still equals the event SHA.
...
The resumed SoP review found that the pre-push hook did not consume Git's pushed-ref stream and therefore inspected only the checked-out `HEAD`; a different ref could bypass that local gate. It also found that event-range checks scanned only the final tree, so a blocked token could be introduced and removed in intermediate commits while remaining in reachable history. The policy now consumes and validates each pre-push ref update, checks pushed ref targets including annotated-tag objects, scans every newly introduced commit object and tree/content, and inspects annotated-tag objects reached by local refs. `docs/TEST_LAB.md` also now records the automatic repository-policy workflow exception consistently. These corrections reset the SoP sequence.

The first restarted pass then found two policy edge cases. The ASCII byte fast path could falsely match a configured token embedded inside valid non-ASCII Unicode word characters even though the normalized text matcher correctly treated it as non-token text. The pre-push path also checked an offending remote ref name before recognizing a deletion, which could prevent removal of a bad ref. Valid UTF-8 content is now decoded and checked with the Unicode whole-token matcher first, the byte matcher is reserved for non-UTF-8 content, the matcher self-test covers Unicode word boundaries, and ref-deletion updates are allowed to remove offending remote names without inspecting the deleted target. These corrections reset the SoP sequence again.

The next complete review found that pre-push still scanned every existing local ref before processing the pushed-ref stream. A local remote-tracking ref with an offending name could therefore block the very deletion push intended to remove that remote ref. Pre-push now checks local configuration plus each non-deletion ref update supplied by Git, while staged/default policy modes retain full local-ref inspection. This correction resets the SoP sequence again.

Three consecutive complete reviews of the resulting tree were clean. Because the connected repository interface does not expose workflow dispatch, the policy workflow now includes a narrowly scoped owner-only issue control that dispatches the existing manual acceptance workflows after policy succeeds. The dispatcher validates that `main` still equals the triggering event SHA before requesting the build, continuity, reference and E1 runs. This infrastructure change resets the SoP sequence once more before those exact-head gates may run.

The first acceptance-dispatch attempt exposed a metadata-event range defect. Events without a commit range treated the current head as an unbounded history scan, so older accepted versions of the policy implementation were rechecked under the current matcher and blocked the dispatcher. Metadata and manual events now inspect the exact current tree plus refs, local configuration, event fields and collaborator data without walking ancestral commit trees; push and pre-push paths continue scanning every newly introduced commit and tree. This correction resets the SoP sequence.

The next acceptance-dispatch attempt passed repository policy but failed before launching acceptance because the command-line client was invoked from a runner workspace with no checkout and no explicit repository selection. The dispatcher now supplies the repository explicitly on every workflow-dispatch command, so no local Git checkout is required. This correction resets the SoP sequence.

After three clean complete reviews, exact-head acceptance was dispatched successfully. Native build and continuity gates passed, but the arm64 E1 job failed while constructing the guest image because Ubuntu Base did not contain a certificate bundle capable of validating the pinned HTTPS package snapshot. The image builder now seeds the runner trust bundle only for the initial snapshot connection, then installs `ca-certificates` from the pinned snapshot and refreshes package-owned trust inside the guest. This correction resets the SoP sequence.

## Resume point

`main` is the only active development line and contains the E1 two-router adjacency acceptance harness plus the image-construction trust bootstrap required to reach the pinned package snapshot from minimal Ubuntu Base. Repository-policy push coverage checks pushed refs and intermediate reachable commit content, deletion pushes can clean up offending remote refs, and metadata/manual events check the exact current tree without re-evaluating accepted ancestral trees. The owner-only acceptance dispatcher explicitly targets the repository and remains the demand-driven path to the manual acceptance workflows. Netdevice filters, hello transmission, address updates and EtherType receive processing are restricted to `init_net`, and every attached Ethernet device owns an explicit DECnet unicast-filter reference independent of its primary MAC. The SoP sequence is reset.

## Next action

Restart the SoP sequence from the exact current `main` HEAD and require three consecutive clean complete passes. Any defect or later edit resets the sequence. Then invoke the owner-only acceptance dispatcher and require green build/reference/continuity gates plus native E1 two-node VM mode on x86_64 and aarch64 against that exact HEAD. If all required evidence is green, add live interoperability against the pinned Route20 and PyDECnet forks before beginning Phase 4 routing work. Continue all work directly on current `main` HEAD.
