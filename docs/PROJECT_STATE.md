...
The first rerun after the resumable-session state repair verified both pinned Alpine images and reached x86 session creation. It then failed before creating NoCloud seeds because `tests/lab/make-nocloud-seed.sh` had mode `0644`; this is corrected to executable. The retained artifact for session `gha-34948701365` is intentionally incomplete: it contains copied guest disks but no valid session manifest/seeds, so the resume checks reject it. It must not be treated as a known resumable pair.
