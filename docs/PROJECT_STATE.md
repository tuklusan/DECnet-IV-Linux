...
Phase 3 remains active. The prior exact candidate `a57f44a2400bbf1fce7cc411489d51182c9ccd95` completed three clean semantic/manual SoP passes, but acceptance child VM run `35181712924` failed before boot on arm64 because direct-boot image construction had no explicit initramfs generator under `--no-install-recommends`. The current `main` candidate corrects that failure by explicitly installing `initramfs-tools`, asserting the generated initrd before cleanup, and running a permanent image-builder regression from both the pre-commit hook and workflow SoP. No SoP pass or acceptance result from the prior candidate carries forward.
