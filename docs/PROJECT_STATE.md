...
The failure evidence exposed two shared provisioning defects rather than protocol failures. On x86_64, Tiny Cloud ran, `linux-virt` and `linux-virt-dev` upgraded together to `6.18.52-r0`, and AKMS installed `decnet_iv` successfully for `6.18.52-0-virt`; compilation of `dnctl` then failed because `/usr/include/linux/ioctl.h` was absent. The guest provisioning payload now installs Alpine's `linux-headers` userspace headers while compiling the tools and removes them with the other transient build dependencies afterward.

On aarch64, the UEFI tiny image booted normally to the Alpine login prompt but Tiny Cloud never executed the provisioning payload. The ISO9660 `CIDATA` seed had been exposed as a generic virtio block device. The seed builder now supports both the existing ISO format and a portable FAT filesystem volume labeled `cidata`; ARM and mixed-architecture jobs select the FAT format and attach it as a virtio block device on both guest architectures. The x86-only and retained-session path keeps the established ISO/CD-ROM behavior, including legacy `.seed.iso` manifest names, so recovery can refresh an incomplete guest without changing the retained QCOW2 identity or breaking old session metadata.
...
Candidate `eb518891197be467a1c8ca7a4c05d20e4e1b8570` was rejected by all four VM gates in workflow run `34958450892`; its non-VM gates were green. The branch now carries the two resulting provisioning repairs: transient `linux-headers` for userspace tool compilation, and FAT `cidata` block seeds for ARM/mixed Tiny Cloud discovery. The implementation still stops at Routing Layer EtherType plumbing: no adjacency, routing, NSP, Session Control, NICE/NML application behavior, or DDCMP is claimed yet. The retained recovery source remains workflow run `34948868114`, session `gha-34948868114`.

When read on a feature branch, freeze and review this repaired candidate under SoP, then finish the exact-commit gates described above. When read from `main`, Phase 2 and the documentation reconciliation have been promoted together and the next development work is Phase 3 Ethernet hello/adjacency on a new feature branch.

## Next action

Freeze the repaired Phase 2 candidate, run three consecutive clean full SoP passes over every updated deliverable, and publish those exact bytes in one substantive commit with this state and generated handover. Require every normal-branch gate on that exact commit to pass, including fresh x86_64/x86_64, aarch64/aarch64, and both mixed CPU VM cases. Then validate the same commit on recovery branch `resume-34948868114` and require its restored x86 session to pass without replacing the retained QCOW2 disks. Only then fast-forward `main` to that exact commit. After promotion, begin Phase 3 on a new feature branch: implement DECnet Ethernet address handling, endnode/router hello parsing and generation, adjacency state/expiry, and independent Route20/PyDECnet interoperability vectors while retaining all Phase 0-2 gates. DECnet-over-IP remains deferred.
