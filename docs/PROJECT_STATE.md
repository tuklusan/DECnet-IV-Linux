...
Candidate `e4b2f23c43d48fe6b4d35ddf6dfcc2ed07a5ff0a` is rejected by the fresh x86_64/x86_64 VM gate in workflow run `34984520980`; all normal non-VM gates on that SHA are green. Both x86 guests reached successful 6.18.52 kernel/header upgrade and AKMS module installation, then Tiny Cloud provisioning aborted because `/usr/local/sbin` was absent when linking `dnctl`. The repair creates the required userspace/service installation directories before compilation and installation. The implementation still stops at Routing Layer EtherType plumbing: no adjacency, routing, NSP, Session Control, NICE/NML application behavior, or DDCMP is claimed yet. The retained recovery source remains workflow run `34948868114`, session `gha-34948868114`.

When read on a feature branch, freeze and review this directory-creation repair under SoP, then finish the exact-commit gates described above. When read from `main`, Phase 2 and the documentation reconciliation have been promoted together and the next development work is Phase 3 Ethernet hello/adjacency on a new feature branch.

## Next action

Freeze the directory-creation repair, run three consecutive clean full SoP passes over every updated deliverable, and publish those exact bytes in one substantive commit with this state and generated handover. Require every normal-branch gate on that exact commit to pass, including fresh x86_64/x86_64, aarch64/aarch64, and both mixed CPU VM cases. Then validate the same commit on recovery branch `resume-34948868114` and require its restored x86 session to pass without replacing the retained QCOW2 disks. Only then fast-forward `main` to that exact commit. After promotion, begin Phase 3 on a new feature branch: implement DECnet Ethernet address handling, endnode/router hello parsing and generation, adjacency state/expiry, and independent Route20/PyDECnet interoperability vectors while retaining all Phase 0-2 gates. DECnet-over-IP remains deferred.
