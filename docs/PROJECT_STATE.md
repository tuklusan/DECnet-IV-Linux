...
- Automated workflows and policy/integrity gates must not use the SoP name or encode the manual delivery-review concept. They perform only mechanical policy, identity and immutability checks.
...
`tools/integrity_scan.py` records machine-readable candidate identity, changed paths/blob hashes and optional full-tree manifests. `tools/workflow_guard.sh` runs policy/regression gates and records a bounded baseline diff manifest which workflow final scans use to detect checkout drift. Those machine checks never replace the complete semantic/manual delivery passes.
...
The first complete-tree pass on `c71bc79a176bbac367a910567160e8809283c051` then found the same obsolete machine-scan description in `scratch/README.md`, which still claimed three automatic complete scans and a three-scan automated gate. Candidate `f2bab342389ce8da8c8696c222c37c35326ceefe` corrected the scratch workspace contract to describe the actual bounded baseline/final machine checks while preserving the separate three-pass complete semantic/manual delivery SoP.
...
The current repository-policy cleanup removes the SoP name and concept from every automated workflow and integrity/policy gate. The former machine helpers are replaced by `tools/workflow_guard.sh` and `tools/integrity_scan.py`, workflow evidence moves from `sop/` to `integrity/`, and regression wording is neutral. The manual delivery SoP remains unchanged and separate. Because this is a substantive repository/acceptance change, the clean manual pass count remains zero.
...
Phase 3 remains active. The corrected image path now follows QEMU AArch64 direct-loader semantics, while automated workflow/policy machinery has been separated completely from the manual SoP terminology and concept. The protocol correction for per-interface designated-router DRDELAY/handoff remains unchanged and still awaits executable E1/independent-peer proof on the corrected image path.
