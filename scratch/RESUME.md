...
Continuity validation reads staged records from the index and committed records from the requested commit object, with a regression test proving working-tree substitutions cannot mask either case. The workflow-budget gate now uses the same exact-source rule: `--staged` reads workflow bytes from the index and `--tree` reads them from the requested commit. Its regression suite proves mutable working-tree files cannot mask an over-budget staged workflow or alter a committed acceptance scan. The gate also bounds artifact parsing by workflow step, enforces the 75-minute ceiling and storage limits, and caps each live interoperability matrix row at two scenarios. All earlier SoP pass counts and acceptance evidence are invalid for the resulting `main` candidate.
...
Every workflow records a `state.json` containing source commit/tree, workflow/job, run ID, run attempt, runner identity, architecture/mode, parent/resume run IDs, milestones and status. The surrounding compact scratch artifact retains scan manifests, logs, packet evidence and small checkpoint metadata. The two-node VM workflow separately retains a short-lived sealed format-2 checkpoint artifact containing the base disk, both node deltas, kernel, initrd, `session.env` and hashes.
