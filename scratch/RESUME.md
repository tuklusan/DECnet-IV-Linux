...
Candidate `a57f44a2400bbf1fce7cc411489d51182c9ccd95` completed three semantic/manual SoP passes, then exact acceptance parent `35181684059` exposed an arm64 image-construction defect in child VM run `35181712924`: the kernel installed, but no initrd was generated because image package installation used `--no-install-recommends` without explicitly installing `initramfs-tools`. The image builder now explicitly installs `initramfs-tools` and asserts `/boot/initrd.img-$krel` before cleanup. The first review of that correction caught a second defect: its pre-commit image-builder gate read the mutable working tree rather than the staged builder. `tests/policy/test_image_builder_gate.py` now has explicit `--staged` and `--tree` source modes; the hook reads the staged index and workflow SoP reads the exact requested commit. The next full semantic/manual pass completed cleanly, but adversarial Pass 2 found that VM checkpoint pruning only inspected the first 100 repository artifacts. The workflow now uses paginated artifact enumeration, and `tools/workflow_budget_gate.py` requires the artifact-list endpoint and pagination safeguard. Every prior SoP count and acceptance result is invalid after this latest correction.
...
| Phase 3 acceptance | prior candidate failed in arm64 image build; later candidates changed; fresh dispatch required |
...
| Latest interoperability run | `35181714307`, prior candidate; ineligible after fixes |
