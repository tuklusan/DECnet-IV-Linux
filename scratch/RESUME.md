...
Phase 3 remains the active protocol phase. Work is performed only on `main`. The commit containing this file adds the repository-root `scratch/` persistence contract, exact-tree SoP scan tooling, and workflow state/evidence persistence. Mutable workflow state is written below ignored `scratch/runtime/` during a run, uploaded as uniquely named workflow artifacts, and restored below ignored `scratch/restored/` on later runner sessions.

The persistence and workflow-gate changes are substantive, so all earlier SoP pass counts and acceptance evidence are invalid for the resulting `main` candidate.
