...
Continuity validation now reads staged records from the index and committed records from the requested commit object, with a regression test proving working-tree substitutions cannot mask either case. The first workflow-budget implementation also exposed a parser defect during review: adjacent artifact-upload steps could be read as one block and produce a false duplicate-retention error. Upload blocks are now bounded by step boundaries and a regression test covers adjacent uploads plus the 75-minute ceiling. These corrections are substantive, so all earlier SoP pass counts and acceptance evidence are invalid for the resulting `main` candidate.
