...
The first acceptance trigger for the ARM wrapper change was rejected before child dispatch because the substantive commit had not refreshed both continuity records. The current candidate corrects that process defect by carrying this file and `docs/PROJECT_STATE.md` in the same substantive commit as the implementation/diagnostic changes.
...
| Infrastructure status | architecture closed; ARM wrapper and Route20 runtime evidence pending exact acceptance |
...
Run exact-head acceptance for the current `main`. Verify the ARM64 foundation kernel is normalized to a raw Image and reaches serial/DECnet execution. If Route20 exits again, use the newly emitted Route20 syslog/kernel diagnostics to correct only the demonstrated runtime fault. When Route20/PyDECnet stay alive, continue Phase 3 from the first genuine protocol/interoperability failure.
