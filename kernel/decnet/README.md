...
Phase 5 registers a native `AF_DECnet` / `SOCK_SEQPACKET` family backed by the in-kernel NSP connection table. The current socket boundary supports bind/getname, outbound connect by DECnet node plus object number/name, blocking and nonblocking connect, record send/receive with NSP segmentation/reassembly, poll wakeups, orderly close, listener registration/backlog, inbound object-number/name matching and `accept()` child sockets. Independent PyDECnet MIRROR and inbound listener interoperability are proven on the accepted Phase 5 candidate. Interrupt/OOB delivery, classic DECnet socket options/access/connect data, stream mode and broader lifecycle negatives remain ordered Phase 5 follow-up work.
