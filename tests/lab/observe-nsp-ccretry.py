...
def main() -> int:
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if nsp is None or len(nsp) < 9 or nsp[0] != CC:
                continue
            now = time.monotonic()
            if first is None:
                first = bytes(nsp)
                first_at = now
                print(
                    "ccretry-observe: first Connect Confirm observed",
                    flush=True,
                )
                continue
            if bytes(nsp) != first:
            ...
            if elapsed < 3.5:
                raise RuntimeError(
                    f"Connect Confirm retransmitted too early: {elapsed:.3f}s"
                )
            print(
                "ccretry-observe: pass duplicate_connect_confirm=1 "
                f"elapsed={elapsed:.3f}s",
                flush=True,
            )
            return 0
        raise RuntimeError("duplicate Connect Confirm not observed")
    finally:
        sniff.close()
