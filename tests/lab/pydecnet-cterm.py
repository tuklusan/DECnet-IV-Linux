...
async def serve(api_socket: str, system: str) -> int:
    ...
    try:
        ...
        dap_request = await dap.recv()
        if dap_request.type != "connect":
            raise RuntimeError(f"expected FAL text-put connect, got {dap_request.type!r}")
        await dap.accept()
        dap_reply = await dap.recv()
        dap_config = bytes(dap_reply)
        if dap_reply.type != "data" or len(dap_config) != 12 or dap_config[0] != 1:
            raise RuntimeError(f"bad text-put DAP CONFIG: {dap_config!r}")
        dap.data(dap_config)

        attrib = await dap.recv()
        if attrib.type != "data" or bytes(attrib) != bytes((2, 0, 0x0f, 1, 0, 2, 2)):
            raise RuntimeError(f"bad text-put DAP ATTRIBUTES: {bytes(attrib)!r}")
        access = await dap.recv()
        if access.type != "data" or b"TEXTUP.TXT" not in bytes(access):
            raise RuntimeError(f"bad text-put DAP ACCESS: {bytes(access)!r}")
        dap.data(bytes((2, 0, 4, 2)))
        dap.data(bytes((6, 0)))

        control = await dap.recv()
        if control.type != "data" or bytes(control) != bytes((4, 0, 2)):
            raise RuntimeError(f"bad text-put DAP CONTROL CONNECT: {bytes(control)!r}")
        dap.data(bytes((6, 0)))
        control = await dap.recv()
        if control.type != "data" or bytes(control) != bytes((4, 0, 4)):
            raise RuntimeError(f"bad text-put DAP CONTROL PUT: {bytes(control)!r}")
        text_records = []
        while len(text_records) < 2:
            data_msg = await dap.recv()
            raw_data = bytes(data_msg)
            if data_msg.type != "data" or len(raw_data) < 3 or raw_data[:3] != bytes((8, 0, 0)):
                raise RuntimeError(f"bad text-put DAP DATA: {raw_data!r}")
            text_records.append(raw_data[3:])
        if text_records != [b"ALPHA", b"BETA"]:
            raise RuntimeError(f"bad text-put records: {text_records!r}")
        accom = await dap.recv()
        if accom.type != "data" or bytes(accom) != bytes((7, 0, 1)):
            raise RuntimeError(f"bad text-put DAP ACCOMP command: {bytes(accom)!r}")
        dap.data(bytes((7, 0, 2)))
        dap.disconnect()
        ...
        dap_request = await dap.recv()
        if dap_request.type != "connect":
            raise RuntimeError(f"expected FAL erase connect, got {dap_request.type!r}")
        await dap.accept()
        dap_reply = await dap.recv()
        dap_config = bytes(dap_reply)
        if dap_reply.type != "data" or len(dap_config) != 12 or dap_config[0] != 1:
            raise RuntimeError(f"bad erase DAP CONFIG: {dap_config!r}")
        dap.data(dap_config)
        ...
        if (
            access.type != "data"
            or len(access_bytes) != 5 + len(expected_name)
            or access_bytes[:5] != bytes((3, 0, 4, 0, len(expected_name)))
            or access_bytes[5:] != expected_name
        ):
            raise RuntimeError(f"bad erase DAP ACCESS: {access_bytes!r}")
        dap.data(bytes((7, 0, 2)))
        dap.disconnect()
        ...
        dap_request = await dap.recv()
        if dap_request.type != "connect":
            raise RuntimeError(f"expected FAL dncopy-get connect, got {dap_request.type!r}")
        await dap.accept()
        dap_reply = await dap.recv()
        dap_config = bytes(dap_reply)
        if dap_reply.type != "data" or len(dap_config) != 12 or dap_config[0] != 1:
            raise RuntimeError(f"bad dncopy-get DAP CONFIG: {dap_config!r}")
        dap.data(dap_config)
        access = await dap.recv()
        if access.type != "data" or b"PHASE7.TXT" not in bytes(access):
            raise RuntimeError(f"bad dncopy-get DAP ACCESS: {bytes(access)!r}")
        dap.data(bytes((2, 0, 4, 4)))
        dap.data(bytes((6, 0)))
        control = await dap.recv()
        if control.type != "data" or bytes(control) != bytes((4, 0, 2)):
            raise RuntimeError(f"bad dncopy-get DAP CONTROL CONNECT: {bytes(control)!r}")
        dap.data(bytes((6, 0)))
        control = await dap.recv()
        if control.type != "data" or bytes(control) != bytes((4, 0, 1)):
            raise RuntimeError(f"bad dncopy-get DAP CONTROL GET: {bytes(control)!r}")
        dap.data(bytes((8, 0, 0)) + b"DAP-PHASE7\r\n")
        dap.data(bytes((9, 0, 0x27, 0x40)))
        accom = await dap.recv()
        if accom.type != "data" or bytes(accom) != bytes((7, 0, 1)):
            raise RuntimeError(f"bad dncopy-get DAP ACCOMP command: {bytes(accom)!r}")
        dap.data(bytes((7, 0, 2)))
        dap.disconnect()
        ...
        dap_request = await dap.recv()
        if dap_request.type != "connect":
            raise RuntimeError(f"expected FAL dncopy-put connect, got {dap_request.type!r}")
        await dap.accept()
        dap_reply = await dap.recv()
        dap_config = bytes(dap_reply)
        if dap_reply.type != "data" or len(dap_config) != 12 or dap_config[0] != 1:
            raise RuntimeError(f"bad dncopy-put DAP CONFIG: {dap_config!r}")
        dap.data(dap_config)
        attrib = await dap.recv()
        if attrib.type != "data" or bytes(attrib) != bytes((2, 0, 0x0f, 1, 0, 2, 2)):
            raise RuntimeError(f"bad dncopy-put DAP ATTRIBUTES: {bytes(attrib)!r}")
        access = await dap.recv()
        if access.type != "data" or b"COPYUP.TXT" not in bytes(access):
            raise RuntimeError(f"bad dncopy-put DAP ACCESS: {bytes(access)!r}")
        dap.data(bytes((2, 0, 4, 2)))
        dap.data(bytes((6, 0)))
        control = await dap.recv()
        if control.type != "data" or bytes(control) != bytes((4, 0, 2)):
            raise RuntimeError(f"bad dncopy-put DAP CONTROL CONNECT: {bytes(control)!r}")
        dap.data(bytes((6, 0)))
        control = await dap.recv()
        if control.type != "data" or bytes(control) != bytes((4, 0, 4)):
            raise RuntimeError(f"bad dncopy-put DAP CONTROL PUT: {bytes(control)!r}")
        copied = []
        while len(copied) < 2:
            data_msg = await dap.recv()
            raw_data = bytes(data_msg)
            if data_msg.type != "data" or len(raw_data) < 3 or raw_data[:3] != bytes((8, 0, 0)):
                raise RuntimeError(f"bad dncopy-put DAP DATA: {raw_data!r}")
            copied.append(raw_data[3:])
        if copied != [b"COPY-A", b"COPY-B"]:
            raise RuntimeError(f"bad dncopy-put records: {copied!r}")
        accom = await dap.recv()
        if accom.type != "data" or bytes(accom) != bytes((7, 0, 1)):
            raise RuntimeError(f"bad dncopy-put DAP ACCOMP command: {bytes(accom)!r}")
        dap.data(bytes((7, 0, 2)))
        dap.disconnect()
        ...
        dap_request = await dap.recv()
        if dap_request.type != "connect":
            raise RuntimeError(f"expected FAL metadata-put connect, got {dap_request.type!r}")
        await dap.accept()
        dap_reply = await dap.recv()
        dap_config = bytes(dap_reply)
        if dap_reply.type != "data" or len(dap_config) != 12 or dap_config[0] != 1:
            raise RuntimeError(f"bad metadata-put DAP CONFIG: {dap_config!r}")
        dap.data(dap_config)
        attrib = await dap.recv()
        if attrib.type != "data" or bytes(attrib) != bytes((2, 0, 0x0f, 1, 0, 3, 4)):
            raise RuntimeError(f"bad metadata-put DAP ATTRIBUTES: {bytes(attrib)!r}")
        access = await dap.recv()
        if access.type != "data" or b"METADATA.TXT" not in bytes(access):
            raise RuntimeError(f"bad metadata-put DAP ACCESS: {bytes(access)!r}")
        dap.data(bytes((2, 0, 4, 3)))
        dap.data(bytes((6, 0)))
        control = await dap.recv()
        if control.type != "data" or bytes(control) != bytes((4, 0, 2)):
            raise RuntimeError(f"bad metadata-put DAP CONTROL CONNECT: {bytes(control)!r}")
        dap.data(bytes((6, 0)))
        control = await dap.recv()
        if control.type != "data" or bytes(control) != bytes((4, 0, 4)):
            raise RuntimeError(f"bad metadata-put DAP CONTROL PUT: {bytes(control)!r}")
        copied = []
        while len(copied) < 2:
            data_msg = await dap.recv()
            raw_data = bytes(data_msg)
            if data_msg.type != "data" or len(raw_data) < 3 or raw_data[:3] != bytes((8, 0, 0)):
                raise RuntimeError(f"bad metadata-put DAP DATA: {raw_data!r}")
            copied.append(raw_data[3:])
        if copied != [b"META-A", b"META-B"]:
            raise RuntimeError(f"bad metadata-put records: {copied!r}")
        accom = await dap.recv()
        if accom.type != "data" or bytes(accom) != bytes((7, 0, 1)):
            raise RuntimeError(f"bad metadata-put DAP ACCOMP command: {bytes(accom)!r}")
        dap.data(bytes((7, 0, 2)))
        dap.disconnect()
        ...
        dap_request = await dap.recv()
        if dap_request.type != "connect":
            raise RuntimeError(f"expected FAL print connect, got {dap_request.type!r}")
        await dap.accept()
        dap_reply = await dap.recv()
        dap_config = bytes(dap_reply)
        if dap_reply.type != "data" or len(dap_config) != 12 or dap_config[0] != 1:
            raise RuntimeError(f"bad print DAP CONFIG: {dap_config!r}")
        dap.data(dap_config)
        attrib = await dap.recv()
        if attrib.type != "data" or bytes(attrib) != bytes((2, 0, 0)):
            raise RuntimeError(f"bad print DAP ATTRIBUTES: {bytes(attrib)!r}")
        access = await dap.recv()
        access_bytes = bytes(access)
        expected_name = b"PRINTME.LIS"
        if (
            access.type != "data"
            or len(access_bytes) != 5 + len(expected_name)
            or access_bytes[:5] != bytes((3, 0, 1, 0, len(expected_name)))
            or access_bytes[5:] != expected_name
        ):
            raise RuntimeError(f"bad print DAP ACCESS: {access_bytes!r}")
        dap.data(bytes((2, 0, 0)))
        dap.data(bytes((6, 0)))
        accom = await dap.recv()
        if accom.type != "data" or bytes(accom) != bytes((7, 0, 1, 0x80, 0x80, 0x40)):
            raise RuntimeError(f"bad print DAP ACCOMP: {bytes(accom)!r}")
        dap.data(bytes((7, 0, 2)))
        dap.disconnect()
