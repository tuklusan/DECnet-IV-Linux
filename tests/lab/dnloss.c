...
#define LOSS_PAYLOAD "DNIV-LOSS-PROBE"
...
int main(int argc, char **argv)
{
    ...
    if (connect(fd, (struct sockaddr *)&peer, sizeof(peer))) {
        perror("connect(MIRROR loss probe)");
        close(fd);
        return 1;
    }

    printf("DNIV-INTEROP-LOSS-READY session=%s scenario=%s peer=%s\n",
           argv[2], argv[3], argv[1]);
    sleep(2);

    tx[0] = 0U;
    memcpy(tx + 1U, LOSS_PAYLOAD, sizeof(LOSS_PAYLOAD) - 1U);
    payload_len = sizeof(LOSS_PAYLOAD);
    sent = send(fd, tx, payload_len, MSG_EOR | MSG_NOSIGNAL);
    if (sent != (ssize_t)payload_len) {
        if (sent < 0)
            perror("send(MIRROR loss probe)");
        else
            fprintf(stderr, "short loss-probe send: %zd/%zu\n", sent,
                    payload_len);
        close(fd);
        return 1;
    }
    ...
    if (got != (ssize_t)payload_len || rx[0] != 1U ||
        ...
        if (got < 0)
            perror("recv(MIRROR loss probe)");
        else
            fprintf(stderr, "bad loss-probe reply: got=%zd expected=%zu\n",
                    got, payload_len);
        close(fd);
        return 1;
    }
    ...
    close(fd);
    printf("dnloss: pass peer=%s payload=%s\n", argv[1], LOSS_PAYLOAD);
    return 0;
}
