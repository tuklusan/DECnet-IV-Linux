...
#define DRAIN_PAYLOAD "DNIV-DRAIN-PROBE"
...
int main(int argc, char **argv)
{
    ...
    if (connect(fd, (struct sockaddr *)&peer, sizeof(peer))) {
        perror("connect(MIRROR drain probe)");
        close(fd);
        return 1;
    }

    printf("DNIV-INTEROP-DRAIN-READY session=%s scenario=%s peer=%s\n",
           argv[2], argv[3], argv[1]);
    sleep(2);

    tx[0] = 0U;
    memcpy(tx + 1U, DRAIN_PAYLOAD, sizeof(DRAIN_PAYLOAD) - 1U);
    payload_len = sizeof(DRAIN_PAYLOAD);
    sent = send(fd, tx, payload_len, MSG_EOR | MSG_NOSIGNAL);
    if (sent != (ssize_t)payload_len) {
        if (sent < 0)
            perror("send(MIRROR drain probe)");
        else
            fprintf(stderr, "short drain-probe send: %zd/%zu\n", sent,
                    payload_len);
        close(fd);
        return 1;
    }
    ...
    if (close(fd)) {
        perror("close(MIRROR drain probe)");
        return 1;
    }
    printf("DNIV-INTEROP-DRAIN-CLOSED session=%s scenario=%s peer=%s\n",
           argv[2], argv[3], argv[1]);
    return 0;
}
