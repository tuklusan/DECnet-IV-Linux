...
if [[ "$reference" == pydecnet ]]; then
    ...
    if ! wait_candidate_marker "$candidate_log" "DNIV-INTEROP-LIBDNET-DAEMON-READY session=$session scenario=$scenario" 30 "$CANDIDATE_PID" "$REFERENCE_PID" "$ref1_log"; then
        tail -240 "$candidate_log" >&2 || true
        tail -180 "$ref1_log" >&2 || true
        exit 1
    fi
    if ! env PYTHONPATH="$host_pydecnet/pydecnet" python3 \
        ...
        "$area.$node" "$ref_name" LIBMIRROR; then
        tail -260 "$candidate_log" >&2 || true
        tail -200 "$ref1_log" >&2 || true
        exit 1
    fi
    if ! wait_candidate_marker "$candidate_log" "DNIV-INTEROP-LIBDNET-DAEMON-PASS session=$session scenario=$scenario" 30 "$CANDIDATE_PID" "$REFERENCE_PID" "$ref1_log"; then
        tail -260 "$candidate_log" >&2 || true
        tail -200 "$ref1_log" >&2 || true
        exit 1
    fi
    ...
    pyhttp_object_log="$work/pydecnet-http-object.log"
    env PYTHONPATH="$host_pydecnet/pydecnet" python3 \
        "$script_dir/pydecnet-http-server.py" "$host_pydecnet_api" "$ref_name" \
        DNIVHT PYDECNET-DNLYNX-OBJECT-PASS >"$pyhttp_object_log" 2>&1 &
    pyhttp_object_pid=$!
    pyhttp_object_ready=0
    for _ in $(seq 1 100); do
        if grep -Fq 'pydecnet-http-server: ready object=DNIVHT' "$pyhttp_object_log" 2>/dev/null; then
            pyhttp_object_ready=1
            break
        fi
        if ! kill -0 "$pyhttp_object_pid" 2>/dev/null; then
            break
        fi
        sleep 0.1
    done
    if [[ "$pyhttp_object_ready" -ne 1 ]]; then
        cat "$pyhttp_object_log" >&2 || true
        wait "$pyhttp_object_pid" 2>/dev/null || true
        exit 1
    fi
    if ! wait_candidate_marker "$candidate_log" "DNIV-INTEROP-DNLYNX-OBJECT-READY session=$session scenario=$scenario" 30 "$CANDIDATE_PID" "$REFERENCE_PID" "$ref1_log"; then
        kill "$pyhttp_object_pid" 2>/dev/null || true
        wait "$pyhttp_object_pid" 2>/dev/null || true
        cat "$pyhttp_object_log" >&2 || true
        exit 1
    fi
    if ! wait_candidate_marker "$candidate_log" "DNIV-INTEROP-DNLYNX-OBJECT-PASS session=$session scenario=$scenario" 30 "$CANDIDATE_PID" "$REFERENCE_PID" "$ref1_log"; then
        kill "$pyhttp_object_pid" 2>/dev/null || true
        wait "$pyhttp_object_pid" 2>/dev/null || true
        cat "$pyhttp_object_log" >&2 || true
        tail -220 "$candidate_log" >&2 || true
    ...
    if ! wait "$pyhttp_object_pid"; then
        cat "$pyhttp_object_log" >&2 || true
        exit 1
    fi
    grep -Fq 'pydecnet-http-server: pass' "$pyhttp_object_log" || {
        cat "$pyhttp_object_log" >&2 || true
        exit 1
    }
    ...
    if ! env PYTHONPATH="$host_pydecnet/pydecnet" python3 \
        "$script_dir/pydecnet-phone.py" "$host_pydecnet_api" \
        "$area.$node" "$ref_name"; then
        tail -220 "$candidate_log" >&2 || true
        tail -160 "$ref1_log" >&2 || true
        exit 1
    fi
    if ! wait_candidate_marker "$candidate_log" "DNIV-INTEROP-PHONE-PASS session=$session scenario=$scenario" 30 "$CANDIDATE_PID" "$REFERENCE_PID" "$ref1_log"; then
        tail -220 "$candidate_log" >&2 || true
        tail -160 "$ref1_log" >&2 || true
        exit 1
    fi
    if ! wait_candidate_marker "$candidate_log" "DNIV-INTEROP-MAIL-READY session=$session scenario=$scenario" 30 "$CANDIDATE_PID" "$REFERENCE_PID" "$ref1_log"; then
        tail -220 "$candidate_log" >&2 || true
        tail -160 "$ref1_log" >&2 || true
        exit 1
    fi
    ...
    if ! wait_candidate_marker "$candidate_log" "DNIV-INTEROP-MAIL-PASS session=$session scenario=$scenario" 30 "$CANDIDATE_PID" "$REFERENCE_PID" "$ref1_log"; then
        tail -220 "$candidate_log" >&2 || true
        tail -160 "$ref1_log" >&2 || true
        exit 1
    fi
