...
is_outer_connect_timeout() {
    grep -Eq '^ssh: connect to host .+ port .+: Connection timed outif [[ "${1:-}" == "--classifier-selftest" ]]; then
    ...
    if is_outer_connect_timeout "$selftest_err"; then
        echo "vde2-cross: inner-timeout classifier false positive" >&2
        exit 1
    fi
...
if [[ "${1:-}" != "--server" && "${1:-}" != "--client" ]]; then
    echo "usage: $0 --preflight-only|--classifier-selftest|--wrapper-selftest|--server|--client" >&2
    exit 2
fi
